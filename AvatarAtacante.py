            ...
            if not self.boton3:                
                    ...
                    while fila <=radio:
                        ...
                        while col<=radio:
                            if cityblock([self.x,self.y],[self.x+col, self.y+fila])<=2:
                                if self.y+fila>=0 and self.x+col>=0 and self.x+col<len(self.mapa[0]) and self.y+fila<len(self.mapa):     
                                    if str(self.mapa[self.y+fila][self.x+col]) in list(objetos_frame.keys()):
                                        ...
                                        #   "Estructura destructible" in (objetos_frame[str(self.x+col) + "," + str(self.y+fila)].__dict__["type"]): 
                                                ####print(objetos_frame[str(self.x+col) + "," + str(self.y+fila)])
                ...
                if len(targets)>0:
                    for t in targets:
                        ...
                        objetos_frame[t].__dict__["salud"]-= 250+((250/20)*self.bonus_ataque)
                        self.danio_causado += (250+((250/self.nivel_max)*self.bonus_ataque))/objetos_frame[t].__dict__["salud_max"]
